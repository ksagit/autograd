...
def lstm_predict(params, inputs):
    ...
    output = [hiddens_to_output_probs(params, hiddens)]

    for input in inputs:  # Iterate over time steps.
        cells, hiddens = propagate_state(params, ag_tuple((hiddens, cells)), input)
        output.append(hiddens_to_output_probs(params, hiddens))
    return output
...
if __name__ == '__main__':
    ...
    def training_loss_grad(*args):
        start = time()
        res = grad(training_loss)(*args)
        end = time()
        print("\nElapsed time: ", end - start)
        return res

    # training_loss_grad = profile(grad(training_loss))
